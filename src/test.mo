...
actor class TestDBUser() = this {
    ...
    public func test() : async ?Text {
        ...
        var afterSize: Nat64 = 0;
                ...
                for (i in Iter.range(1, 1200)) {
                    if (i % 100 == 0) {
                        switch (await EkvmModule._get(ekvm, "foo" # Nat.toText(i-99))) {
                            case (?foo)
                    ...
                    ignore await EkvmModule._put(ekvm, "foo" # Nat.toText(i), Text.encodeUtf8("bar" # Nat.toText(i)), false);
                };
    ...
    public func get(k: Text) : async ?Text {
        switch(db) {
            case(?ekvm) { 
                switch (await EkvmModule._get(ekvm, k)) {
    ...
    public func whereIs2(k: Text) : async ?Principal {
        ...
        switch(db) {
            case(?ekvm) { 
                ...
                switch(EkvmModule._whoManages(ekvm, k)) {
                    case(?can) {
    ...
    public func put(k: Text, v: Text, forceExternal: Bool) : async Bool {
        switch(db) {
            case(?ekvm) { await EkvmModule._put(ekvm, k, Text.encodeUtf8(v), forceExternal) };
